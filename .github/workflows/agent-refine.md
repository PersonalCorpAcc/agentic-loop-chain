---
# Managed by @plainconceptsplatform/workflows@0.5.1. Source: loops/workflows/agent-refine.md. Profile digest: 08448fbaf1f8. Update with `workflows update --force`; consumer edits may be overwritten.
env:
    ...
    steps:
      ...
      - name: Record the outcome
        ...
        with:
          ...
          detail: "${{ job.status == 'success' && 'The issue was refined and its labels moved on.' || 'The refine conclusion did not finish; the step log says where it stopped.' }}"
  incomplete:
...
3. Explore before you write. Call skill("${{ env.PLAN_EXPLORE_SKILL }}") and hold its stance for this step:
...
4. **Classify the change complexity.** Based on your exploration, determine whether this is a
   ...
   **4a. Trivial path.** Skip `/${{ env.PLAN_STORY_COMMAND }}`. Do not write Gherkin acceptance criteria or
...
5. Before writing the story, verify coverage: list every work unit and confirm each one has
   ...
   and explore it now. Then call skill("${{ env.PLAN_STORY_SKILL }}") and run `/${{ env.PLAN_STORY_COMMAND }}` for the issue,
...
flowchart TD
    ...
    refExplore("Explore<br/>${{ env.PLAN_EXPLORE_SKILL }} per work unit,<br/>self-answer, bounded") --> refClassify
    ...
    refStory("Story<br/>/${{ env.PLAN_STORY_COMMAND }}, grounded in the code") -->|✓| refProse
    refStory -.->|✗| refFail

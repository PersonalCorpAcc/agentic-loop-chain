---
# Managed by @plainconceptsplatform/workflows@0.5.1. Source: loops/workflows/agent-triage.md. Profile digest: 08448fbaf1f8. Update with `workflows update --force`; consumer edits may be overwritten.
env:
    ...
    steps:
      ...
      - name: Record the outcome
        ...
        with:
          ...
          detail: "${{ job.status == 'success' && 'The issue was triaged and its labels moved on.' || 'The triage conclusion did not finish; the step log says where it stopped.' }}"
  incomplete:
...
steps:
  ...
  - name: Load open issues for duplicate detection
    ...
    with:
      token: ${{ github.token }}
      # Required by the action and never passed: the duplicate check queried an
      # empty repository name and could never have found a duplicate.

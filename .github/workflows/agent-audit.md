---
# Managed by @plainconceptsplatform/workflows@0.5.1. Source: loops/workflows/agent-audit.md. Profile digest: 08448fbaf1f8. Update with `workflows update --force`; consumer edits may be overwritten.
env:
    ...
    steps:
      ...
      - name: Record the outcome
        ...
        with:
          ...
          route: audit
          subject: '-'
          detail: "${{ job.status == 'success' && 'The audit ran and its findings are on the audit issue.' || 'The audit conclusion did not finish; the step log says where it stopped.' }}"
...
1. Call skill("${{ env.REPO_AUDIT_SKILL }}"), then run `/${{ env.REPO_AUDIT_COMMAND }}` as a read-only audit of this
...
5. Call `create_issue` **once** with title "Audit: <date>". Do NOT specify labels , the
   ...
   **Section 2 , Top 3 to implement:** Call skill("${{ env.PLAN_STORY_SKILL }}") and refine the top 3
...
flowchart TD
    ...
    auditRun("Audit<br/>/${{ env.REPO_AUDIT_COMMAND }}, read-only") -->|✓| auditTriage
    auditRun -.->|✗| auditFail

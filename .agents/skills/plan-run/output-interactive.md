# Output procedure (Interactive modes only: default, push, pr. Never loaded in unattended mode.)
...
Push the feature branch, then load `ops-ship` to create a PR into `$DEFAULT_BRANCH`. Supply title, change id, functional summary, delivered acceptance criteria, task count, verification result, archive path, and commits. Do not merge the PR.

Restore the stash.

## Final report

Print the report below, then the outcome line from [outcome-line.md](outcome-line.md) as the very last line.

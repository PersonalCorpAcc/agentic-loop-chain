...
This is the `plan-story` capability of the loop's contract. It writes a repository-aware user story with acceptance criteria grounded in real paths, components and data models. It writes no files, creates no branches and produces no generic placeholder text.

---
name: repo-verify
description: Verify and repair current-branch changes using applicable fullstack engineer abilities, project checks, and dependency rules. Invoked by /repo-verify and the plan-run pipeline.
license: MIT
---
...
This is the `repo-verify` capability of the loop's contract. It verifies and repairs the current branch against the repository's own checks and dependency rules. It never switches branches, pushes, opens pull requests, creates backlog items or contacts external services.

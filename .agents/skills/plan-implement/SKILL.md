---
name: plan-implement
description: Implement tasks from a plan. OpenSpec-annotated tasks (from plan-propose) run as parallel subagent waves; Todo pane tasks (from /plan-quick) run sequentially in-session. Load when implementing a prepared plan. Invoked by the /plan-implement command (interactive) and the plan-run pipeline (autonomous).
license: MIT
---
...
This is the `plan-implement` capability of the loop's contract. It implements the tasks in a prepared plan, in parallel where the plan marks them independent. It never implements tasks the plan did not declare, and with `start_from: load-plan` it creates no branch and works on the current HEAD.
...
- `autonomous`: do not return control between waves; keep looping until every task is DONE or the progress guard / retry limit trips. On a stall or exhausted retry, stop the wave loop and report to the caller (whose failure policy governs). When all tasks are DONE, the APPLY stage is complete. Hand control back to the caller (the `/plan-run` pipeline) so it continues with the next phase. Do not end the turn here; "report N/N tasks" is a stage boundary, not a finish line.
...
2. If found and tasks have `<!-- agent` annotations (written by `plan-propose`): OpenSpec mode. Follow the protocol below. Annotated tasks run in subagent waves. They never become sequential lead work because the task count seems small, the lead prefers direct implementation, or a worker has not yet been inspected.
...
**1. Branch.** Create `feature/{change-slug}` if not already on one. Skip this step entirely when the caller passed `start_from: load-plan`: then no branch is created or checked, and the current HEAD, named or detached, is where every commit lands.

**2. Load the plan and workers.** Parse `tasks.md`. Each task carries `<!-- agent, depends_on, touches -->` (from `plan-propose`). Inspect `.opencode/agents/` for each base engineer and its generated `.<tier>.md` variants. The tier-suffixed name in an annotation (for example, `backend-engineer.build`) is the worker to spawn: `subagent-tiers` resolves its model at startup and registers it as `mode: subagent`. Read `.opencode/harness.json` -> `agents.maxConcurrent` (the wave cap, 1 to 5).
...
<!-- HARNESS-OPTIMISATION-CODEGRAPH-START -->
<!-- HARNESS-OPTIMISATION-CODEGRAPH-END -->
...
- `subagent_type` = the task's `agent` exactly as written in `tasks.md` (e.g. `frontend-engineer.build`, `backend-engineer.fast`). It is the tier-suffixed `mode: subagent` worker created by `subagent-tiers`. Worker resolution happened in step 2; a missing worker stops the stage before spawning. `fullstack-engineer`, `general`, and the lead session are not fallbacks for annotated implementation work.
...
- `prompt` must contain the exact task IDs and text plus the gathered context. The worker follows the Engineer workflow defined in `@guardrails-generic`; do not restate it in the prompt. **Worker output discipline:** instruct each worker to return a compact summary: what it changed (file names, not contents), pass/fail status of any verification it ran, and any blockers. Workers must NOT return file contents or full diffs as their result — the lead can `git diff` if needed.

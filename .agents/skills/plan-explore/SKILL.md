---
name: plan-explore
description: Explore an idea or requirement before planning. Invoked by the /plan-explore command.
license: MIT
---

**READ-ONLY MODE.** From the moment this skill is loaded until the user explicitly invokes a different command (e.g. `/plan-implement`) or explicitly requests implementation, you MUST NOT write, edit, or create any file, including OpenSpec artifacts. You may only read, search, and discuss. If the conversation drifts toward implementation, remind the user that explore mode is active and suggest `/plan-implement` to start implementing. This overrides any permissive stance in `@openspec-explore` about creating OpenSpec artifacts being "fine."

Load `@openspec-explore` and follow every step defined in it.

<!-- HARNESS-OPTIMISATION-MEMORY-START -->
<!-- HARNESS-OPTIMISATION-MEMORY-END -->

## Guarantees

This is the `plan-explore` capability of the loop's contract. It investigates an idea or requirement before planning. It never writes, edits or creates any file, including planning artefacts.

...
If a work item URL or issue key is provided (GitHub Issue, Azure DevOps work item, Jira issue, or browser-based backlog): load `@userstory` skill and fetch the work item before continuing. Backlog platform is set in `.opencode/harness.json` -> `platform.backlog`. If backlog platform is `none`, skip this step and work from direct input.
  ...
  "questions": [
    {
      ...
      "options": [
        { "label": "yes", "description": "Load the plan-implement skill to start implementation." },
        { "label": "no", "description": "Stop here. You can run /plan-implement later." }
      ]
    }
  ]
}
...
Loading `plan-implement` requires explicit user confirmation.

Autonomous mode: do not call the `question` tool. The PROPOSE stage is complete. Hand the change slug and task count back to the caller (the `/plan-run` pipeline) so it immediately continues to the apply phase. This is a stage boundary, not the end of the run.

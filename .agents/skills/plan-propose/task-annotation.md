...
The tier suffix is appended to the agent name with a dot (e.g. `backend-engineer.build`). This is the agent name you write in the annotation. The `subagent-tiers` plugin resolves the model at startup from `models[<tier>]`.
...
Derive `touches` for each task: the file path(s)/glob(s) it will create or modify (the task text usually names them, e.g. "Modify src/board/components/CreateForm.tsx"). This lets `plan-implement` serialize same-file tasks that have no logical dependency. Include net-new files.
...
Example result (note same-file tasks like 1.1/1.2 share `touches`, so `plan-implement` runs them sequentially even with no `depends_on` between them; tier suffix encodes the model):
...
`plan-implement` reads these annotations to build conflict-free waves: `depends_on` gates ordering, `touches` keeps concurrent agents file-disjoint, and the tier suffix in `agent` determines the model (resolved at startup by the `subagent-tiers` plugin). `depends_on` is mandatory; `touches` is a best-effort hint.
